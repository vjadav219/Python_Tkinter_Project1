...
def csv_A():  
    ...
    df_csv=df.to_csv(f"attendance_of_class_A{b}.csv")
...
def csv_B(): 
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_B{b}.csv")
    tmsg.showinfo("Download","Download Successfully")
def csv_C():   
    global b 
    import pandas as pd
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_C{b}.csv")
    tmsg.showinfo("Download","Download Successfully")
def csv_D():    
    global b
    new=b
    ...
    dictionary={'Roll_Number':w, 'Student_Name':x, 'Status':y, 'date': z}
    df=pd.DataFrame(dictionary)
    df_csv=df.to_csv(f"attendance_of_class_D{new}.csv")
    tmsg.showinfo("Download","Download Successfully")
...
Label(root,text="Guided By Snehal Sathvara",justify=CENTER,font="lucida 11 bold").pack(pady=10)
